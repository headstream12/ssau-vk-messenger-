...
+ (NSString*)sqliteLibVersion {
    return [NSString stringWithFormat:@"%s", bx_unicode_sqlite3_libversion()];
}

+ (BOOL)isSQLiteThreadSafe {
    // make sure to read the sqlite headers on this guy!
    return bx_unicode_sqlite3_threadsafe() != 0;
}

- (id)initWithPath:(NSString*)aPath {
    
    assert(bx_unicode_sqlite3_threadsafe()); // whoa there big boy- gotta make sure sqlite it happy with what we're going to do.
...
- (bx_unicode_sqlite3*)sqliteHandle {
    return _db;
}
...
- (BOOL)close {
    ...
    do {
        retry   = NO;
        rc      = bx_unicode_sqlite3_close(_db);
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                bx_unicode_sqlite3_stmt *pStmt;
                while ((pStmt = bx_unicode_sqlite3_next_stmt(_db, 0x00)) !=0) {
                    NSLog(@"Closing leaked statement");
                    bx_unicode_sqlite3_finalize(pStmt);
                }
            }
        }
...
- (BOOL)rekey:(NSString*)key {
    ...
    int rc = bx_unicode_sqlite3_rekey(_db, [key UTF8String], (int)strlen([key UTF8String]));
...
- (BOOL)setKey:(NSString*)key {
    ...
    int rc = bx_unicode_sqlite3_key(_db, [key UTF8String], (int)strlen([key UTF8String]));
...
- (NSString*)lastErrorMessage {
    return [NSString stringWithUTF8String:bx_unicode_sqlite3_errmsg(_db)];
}
...
- (int)lastErrorCode {
    return bx_unicode_sqlite3_errcode(_db);
}
...
- (sqlite_int64)lastInsertRowId {
    ...
    sqlite_int64 ret = bx_unicode_sqlite3_last_insert_rowid(_db);
...
- (int)changes {
    ...
    int ret = bx_unicode_sqlite3_changes(_db);
...
- (void)bindObject:(id)obj toColumn:(int)idx inStatement:(bx_unicode_sqlite3_stmt*)pStmt {
    
    if ((!obj) || ((NSNull *)obj == [NSNull null])) {
        bx_unicode_sqlite3_bind_null(pStmt, idx);
    }
    ...
    // FIXME - someday check the return codes on these binds.
        ...
        bx_unicode_sqlite3_bind_blob(pStmt, idx, bytes, (int)[obj length], SQLITE_STATIC);
    }
    else if ([obj isKindOfClass:[NSDate class]]) {
        bx_unicode_sqlite3_bind_double(pStmt, idx, [obj timeIntervalSince1970]);
    }
    else if ([obj isKindOfClass:[NSNumber class]]) {
        
        if (strcmp([obj objCType], @encode(BOOL)) == 0) {
            bx_unicode_sqlite3_bind_int(pStmt, idx, ([obj boolValue] ? 1 : 0));
        }
        else if (strcmp([obj objCType], @encode(int)) == 0) {
            bx_unicode_sqlite3_bind_int64(pStmt, idx, [obj longValue]);
        }
        else if (strcmp([obj objCType], @encode(long)) == 0) {
            bx_unicode_sqlite3_bind_int64(pStmt, idx, [obj longValue]);
        }
        else if (strcmp([obj objCType], @encode(long long)) == 0) {
            bx_unicode_sqlite3_bind_int64(pStmt, idx, [obj longLongValue]);
        }
        else if (strcmp([obj objCType], @encode(unsigned long long)) == 0) {
            bx_unicode_sqlite3_bind_int64(pStmt, idx, (long long)[obj unsignedLongLongValue]);
        }
        else if (strcmp([obj objCType], @encode(float)) == 0) {
            bx_unicode_sqlite3_bind_double(pStmt, idx, [obj floatValue]);
        }
        else if (strcmp([obj objCType], @encode(double)) == 0) {
            bx_unicode_sqlite3_bind_double(pStmt, idx, [obj doubleValue]);
        }
        else {
            bx_unicode_sqlite3_bind_text(pStmt, idx, [[obj description] UTF8String], -1, SQLITE_STATIC);
        }
    }
    else {
        bx_unicode_sqlite3_bind_text(pStmt, idx, [[obj description] UTF8String], -1, SQLITE_STATIC);
...
- (BxFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        do {
            retry   = NO;
            rc      = bx_unicode_sqlite3_prepare_v2(_db, [sql UTF8String], -1, &pStmt, 0);
    ...
    id obj;
    int idx = 0;
    int queryCount = bx_unicode_sqlite3_bind_parameter_count(pStmt); // pointed out by Dominic Yu (thanks!)
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (outErr) {
                    *outErr = [self errorWithMessage:[NSString stringWithUTF8String:bx_unicode_sqlite3_errmsg(_db)]];
                }
    ...
    id obj;
    int idx = 0;
    int queryCount = bx_unicode_sqlite3_bind_parameter_count(pStmt);
    ...
    /* Call bx_unicode_sqlite3_step() to run the virtual machine. Since the SQL being
     ** executed is not a SELECT statement, we assume no data will be returned.
     */
    numberOfRetries = 0;
    
    do {
        rc      = bx_unicode_sqlite3_step(pStmt);
        retry   = NO;
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            retry = YES;
            if (SQLITE_LOCKED == rc) {
                rc = bx_unicode_sqlite3_reset(pStmt);
                if (rc != SQLITE_LOCKED) {
                    NSLog(@"Unexpected result from bx_unicode_sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            NSLog(@"Error calling bx_unicode_sqlite3_step (%d: %s) SQLITE_ERROR", rc, bx_unicode_sqlite3_errmsg(_db));
            NSLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            NSLog(@"Error calling bx_unicode_sqlite3_step (%d: %s) SQLITE_MISUSE", rc, bx_unicode_sqlite3_errmsg(_db));
            NSLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            NSLog(@"Unknown error calling bx_unicode_sqlite3_step (%d: %s) eu", rc, bx_unicode_sqlite3_errmsg(_db));
            NSLog(@"DB Query: %@", sql);
        }
    ...
    if (cachedStmt) {
        ...
        closeErrorCode = bx_unicode_sqlite3_reset(pStmt);
    }
    else {
        /* Finalize the virtual machine. This releases all memory and other
         ** resources allocated by the bx_unicode_sqlite3_prepare() call above.
         */
        closeErrorCode = bx_unicode_sqlite3_finalize(pStmt);
    }
    
    if (closeErrorCode != SQLITE_OK) {
        NSLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, bx_unicode_sqlite3_errmsg(_db));
        NSLog(@"DB Query: %@", sql);
    }
...
#else
    void (^block)(bx_unicode_sqlite3_context *context, int argc, bx_unicode_sqlite3_value **argv) = (__bridge id)bx_unicode_sqlite3_user_data(context);
#endif
    block(context, argc, argv);
}


- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(bx_unicode_sqlite3_context *context, int argc, bx_unicode_sqlite3_value **argv))block {
...
- (void)close {
    if (_statement) {
        bx_unicode_sqlite3_finalize(_statement);
        _statement = 0x00;
    }
}
...
- (void)reset {
    if (_statement) {
        bx_unicode_sqlite3_reset(_statement);
    }
}
