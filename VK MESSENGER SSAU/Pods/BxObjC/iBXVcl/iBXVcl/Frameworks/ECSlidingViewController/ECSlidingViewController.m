...
const NSTimeInterval animationDuration = 0.25f;

@interface ECSlidingViewController()
{
@protected
    BOOL isRotateAllowed;
    CGFloat lastTopCenter;
}
...
@property (nonatomic, retain) UIPanGestureRecognizer *topViewSnapshotPanGesture;
...
- (void)setTopViewController:(UIViewController *)theTopViewController
{
    ...
    lastTopCenter = _topViewController.view.center.x;
    
    if (theTopViewController) {
        [self updateChangeTopView];
    }
}
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    NSLog(@"willAnimateRotationToInterfaceOrientation is executed");
    [self adjustLayout];
}
...
- (void)adjustLayout
{
    ...
    if ([self underRightShowing] && ![self topViewIsOffScreen]) {
        NSLog(@"adjustLayout is executed ...");
        [self updateUnderRightLayout];
        ...
        NSLog(@"adjustLayout is executed [self underLeftShowing] && ![self topViewIsOffScreen]");
        [self updateUnderLeftLayout];
        [self updateTopViewHorizontalCenter:self.anchorRightTopViewCenter];
    } else if ([self underLeftShowing] && [self topViewIsOffScreen]) {
        NSLog(@"adjustLayout [self underLeftShowing] && [self topViewIsOffScreen]");
        [self updateUnderLeftLayout];
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.initialHoizontalCenter = lastTopCenter;
...
{
    NSLog(@"resetTopViewWithAnimations");
    isRotateAllowed = NO;
...
- (void)updateTopViewHorizontalCenter:(CGFloat)newHorizontalCenter
{
    ...
    if (self.topResizing == BxTopResizingNone) {
        self.topView.layer.position = center;
    } else {
        CGFloat shiftX = newHorizontalCenter - self.view.bounds.size.width / 2.0f;
        if (shiftX < 0) {
            if (((_topResizing | BxTopResizingRight) == self.topResizing)) {
                self.topView.frame = CGRectMake(0, 0, self.view.bounds.size.width + shiftX, self.view.bounds.size.height);
            } else {
                self.topView.layer.position = center;
            }
        } else if (shiftX > 0) {
            if ((_topResizing | BxTopResizingLeft) == self.topResizing) {
                self.topView.frame = CGRectMake(shiftX, 0, self.view.bounds.size.width - shiftX, self.view.bounds.size.height);
            } else {
                self.topView.layer.position = center;
            }
        } else {
    ...
    lastTopCenter = newHorizontalCenter;
}
...
- (void)topViewHorizontalCenterDidChange:(CGFloat)newHorizontalCenter
{
    ...
    //self.topView.layer.bounds = CGRectMake(0, 0, self.view.bounds.size.width - 320, self.topView.layer.bounds.size.height);
...
- (void)addTopViewSnapshot
{
    if (!self.topViewSnapshot.superview && !self.shouldAllowUserInteractionsWhenAnchored) {
        ...
        if (self.shouldAddPanGestureRecognizerToTopViewSnapshot && (_resetStrategy & ECPanning)) {
            if (!self.topViewSnapshotPanGesture) {
                self.topViewSnapshotPanGesture = [[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(updateTopViewHorizontalCenterWithRecognizer:)] autorelease];
            }
            [topViewSnapshot addGestureRecognizer: self.topViewSnapshotPanGesture];
        }
